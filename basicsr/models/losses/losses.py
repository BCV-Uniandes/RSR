...
from basicsr.models.archs.resnet_arch import ResNetLoss
...
class RobustResNet(nn.Module):
    ...
    def __init__(self,
        ...
        dataset = DATASETS[dataset](data_path)
        self.robresnet = make_and_restore_model(arch=arch,
                                                 dataset=dataset,
                                                 resume_path=resume)
        self.perceptual_weight = perceptual_weight
        self.style_weight = style_weight
    ...
    def forward(self, x, gt):
        ...
        """
        # extract vgg features
        _, x_features = self.robresnet(x)
        _, gt_features = self.robresnet(gt.detach())
        ...
        if self.style_weight > 0:
            ...
            if self.criterion_type == 'fro':
                style_loss += torch.norm(
                    self._gram_mat(x_features) -
                    self._gram_mat(gt_features),
                    p='fro') 
            else:
                style_loss += self.criterion(
                    self._gram_mat(x_features),
